function [sys,x0,str,ts] = sfun_onephase_2L_A(t,x,u,flag,theta_phase,t_dead,t_min,type_signal,asym)
...
dt_plot = 1.0e-10;   % sets the time between two following calculation steps
...
if k == 0
  if type_signal == 2
     u_st_a = U_st*(cos(ksi_s+theta_phase)-cos(3*ksi_s)/6);
  else    % type_signal == 1 or any other value 
     u_st_a = U_st*cos(ksi_s+theta_phase);                 % "default" 
  end % end if
